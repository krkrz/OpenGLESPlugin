 ...
 * プラグインクラスのインスタンス化時にTVPInitializeOpenGLPluginを毎回呼び出す実装にして、Windowクラスのコンストラクタ内でクラス生成すれば問題ない。
 */
extern "C" void* eglGetProcAddress( const char* name ) {
	return TVPeglGetProcAddress(name);
}

#define FIND_PROC(s,type) (s = (type)eglGetProcAddress( #s ))
...
//---------------------------------------------------------------------------
static bool TVPOpenGLESInit = false;
//---------------------------------------------------------------------------
bool TVPInitializeOpenGLPlugin() {
	if( TVPOpenGLESInit == false ) {
		TVPOpenGLESInit = LoadLibGLES();
	}
	return TVPOpenGLESInit;
}
//---------------------------------------------------------------------------
